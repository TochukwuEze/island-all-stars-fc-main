...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const videoItem = galleryItems.find((item) => item.id === slug && item.type === "video");
  
  if (!article && !videoItem) return { title: "Article Not Found | Island Football Club" };
  
  return {
    title: `${article ? article.title : videoItem?.title} | Island Football Club`,
    description: article ? article.content.substring(0, 160) + "..." : (videoItem?.title || ""),
  };
}
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 bg-black text-white py-16 px-6 md:px-12 lg:px-24">
        <div className="max-w-4xl mx-auto">
          <Link 
            href="/" 
            className="inline-flex items-center gap-2 text-gray-400 hover:text-[#2052DA] transition-colors mb-12 text-xs font-bold uppercase tracking-widest group"
          >
          ...
          <header className="mb-12">
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tight uppercase leading-[1.1] mb-6">
              {title}
            </h1>
          ...
          {!isVideo && (
            <article className="prose prose-invert prose-lg max-w-none">
              {article!.content.split('\n\n').map((paragraph, index) => (
                <p key={index} className="text-gray-300 leading-relaxed mb-6 font-light text-lg">
                  {paragraph}
                </p>
              ))}
            </article>
